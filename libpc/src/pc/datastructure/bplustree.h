...
#define LEAF_NODE_FALSE_FLAG 0
    ...
    public:
      ...
      int aggregate_time_stat(const uint64_t key, uint64_t len, long &result);
     ...
     * 3 min
     */
    // int BPlusTree::aggregate(const uint64_t key, int len, long &result, int agg_type) {
    //   check();
    //
    //   struct bplustree_node *current_node = root;
    //
     ...
     */
    int BPlusTree::insert(const uint64_t key, void *value)
    {
    ...
    // void BPlusTree::print_first_layer_layout(std::string filepath, bool size_only = false) {
    //     std::ofstream fout(filepath);
    //     for(int i = 0; i < BPLUSTREE_MAX_KEYS; i++) {
    //         fout << "Key: " << root->entires[i].key << std::endl;
    //     }
    //     for(int i = 0; i < BPLUSTREE_MAX_CHILDREN; i++) {
    //         fout << "Children: " << root->entires[i].key << std::endl;
    //     }
    //
    //
    //     fout.close();
