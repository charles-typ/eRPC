...
class HashTableList {
 public:
  ...
  Node *read() {
    Node *ret = read_cur;
    if (read_cur != NULL) read_cur = read_cur->next;
    return ret;
  }
};
...
class HashTable {
 ...
 public:
  ...
  void insert(std::pair<key_type, const char *> pr) {
    ...
    // << " nanoseconds find location" << std::endl; if (itr != end())
    //{
    ...
    // std::cout << "Inserting key: " << node->key << " into bucket: " <<
    // hash(node->key) << std::endl;
    size++;
    // auto end_time_1 = std::chrono::high_resolution_clock::now();
    // std::cout << "Takes " <<
    // static_cast<uint64_t>(std::chrono::duration_cast<std::chrono::nanoseconds>(end_time_1-end_time_0).count())
    // << " nanoseconds to insert" << std::endl;

    // return Iterator(&this->lists, hash(node->key),
    // this->lists[hash(node->key)].find(node));
    return;
  }
    ...
    for (auto &list : lists) {
      for (Node *n : list) {
        layout << "key: " << n->key << " value: " << n->value
               << " hash: " << hash(n->key)
               << " address: " << static_cast<void *>(&(n->key)) << std::endl;
  ...
  void print_layout_test() {
    ...
    for (auto &list : lists) {
      for (Node *n : list) {
        // layout  << "key: " << n->key << " value: " << n->value << " hash: "
        // << hash(n->key) << " address: " << (void *)&(n->key) << " next: " <<
        // (void*)(*(uint64_t*)((uint64_t)(&(n->key)) + 24)) << std::endl;
        layout << "key: " << n->key << " value: " << n->value
               << " hash: " << hash(n->key)
               << " address: " << static_cast<void *>(&(n->key))
               << " next: " << static_cast<void *>((n->next)) << std::endl;
        // layout  << "key: " << (void *)&(n->key) << " value: " << (void
        // *)&(n->value) << std::endl;
      }
    }
