...
namespace pc
{
  namespace network
  {
                               ...
                               int thread_num, size_t pkt_len)
    {
      ...
      //  LOG(log_level::warn) << std::string(buf);
      //}
      ...
      int overhead;
      char buf[1024];
      sprintf(buf, "Device max mtu %d rx max pktlen %d pkt size %zu",
      ...
      LOG(log_level::info) << "Device overhead " << overhead << " offset " << kPayloadOffset;
      ...
      {
        LOG(log_level::info) << "Setting max rx pkt size to << " << desired_max_size;
      ...
      {
        LOG(log_level::info) << "RTE_ETH_TX_OFFLOAD_MBUF_FAST_FREE enabled";
        port_conf.txmode.offloads |= RTE_ETH_TX_OFFLOAD_MBUF_FAST_FREE;
      }
      if ((dev_info.tx_offload_capa & RTE_ETH_TX_OFFLOAD_IPV4_CKSUM) ==
          RTE_ETH_TX_OFFLOAD_IPV4_CKSUM)
      {
        LOG(log_level::info) << "RTE_ETH_TX_OFFLOAD_IPV4_CKSUM  enabled";
        port_conf.txmode.offloads |= RTE_ETH_TX_OFFLOAD_IPV4_CKSUM;
      }
      if ((dev_info.tx_offload_capa & RTE_ETH_TX_OFFLOAD_UDP_CKSUM) ==
          RTE_ETH_TX_OFFLOAD_UDP_CKSUM)
      {
        LOG(log_level::info) << "RTE_ETH_TX_OFFLOAD_UDP_CKSUM enabled";
      ...
      if (retval != 0)
      {
        LOG(log_level::info) << "Error in rte_eth_dev_configure";
        return retval;
      }
      retval = rte_eth_dev_adjust_nb_rx_tx_desc(port, &nb_rxd, &nb_txd);
      if (retval != 0)
      {
        LOG(log_level::info) << "Error in rte_eth_dev_adjust_nb_rx_tx_desc";
        return retval;
      }
      ...
      sprintf(buf, "NIC %u Socket: %d, MAC: %02" PRIx8 " %02" PRIx8 " %02" PRIx8
      ...
      {
        LOG(log_level::info) << "Waiting for link up on NIC " << port;
        std::this_thread::sleep_for(std::chrono::seconds(1));
        rte_eth_link_get_nowait(port, &link);
      }
      if (link.link_status == 0)
      {
        LOG(log_level::info) << "Link down on NIC " << port;
        return 0;
      }
      LOG(log_level::info) << "Link up on NIC " << port;
      return 0;
    }
    ...
    void DpdkTransport::DpdkInit(uint16_t core_offset, size_t thread_num)
    {
      ...
      LOG(log_level::info) << "Dpdk init on core start " << core_offset << ", num threads " << thread_num;
      int ret = rte_eal_init(rte_argc, const_cast<char **>(rte_argv));
      RtAssert(
          ret >= 0,
          "Failed to initialize DPDK.  Are you running with root permissions?");
      LOG(log_level::info) << rte_version() <<  "initialized";
    }
